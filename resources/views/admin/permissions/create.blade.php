                    ...
                    <form class="pt-3" method="POST" action="{{ route('admin.permissions.store') }}" novalidate>
                        ...
                        <div class="form-group">
                            ...
                            <select id="permission_group" name="permission_group"
                                    ...
                                    @if(Auth::user()->can('permissionGroups.create')) aria-describedby="permissionGroupHelp" @endif>
                                <option value="">Select Group</option>
                                @foreach($permissionGroups as $group)
                                    <option value="{{ $group->id }}"
                                            {{ (old('permission_group') && old('permission_group') == $group->id || Request::query('permissionGroup') === $group->uuid) ? 'selected' : null }}>
                                        {{ $group->name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            @can('permissionGroups.create')
                                <small id="permissionGroupHelp" class="form-text text-muted pt-2 px-4">
                                    <a href="{{ route('admin.permissionGroups.create') }}"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="display_name" name="display_name"
                                   ...
                                   value="{{ old('display_name') }}" required placeholder="Permission">
                            @error('display_name')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name"
                                   ...
                                   value="{{ old('name') }}" required placeholder="Route">
                            @error('name')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
