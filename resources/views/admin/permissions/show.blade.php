                    ...
                    @canany(['permissions.delete', 'permissions.edit'])
                        ...
                        <div class="clearfix">
                            @if (Auth::user()->can('permissions.delete') && !in_array($permission->name, $userPermissions))
                                <a href="{{ route('admin.permissions.delete', ['permission' => $permission->uuid]) }}"
                                   class="float-left btn btn-danger">Delete</a>
                            @endif
                            @can('permissions.edit')
                                <a href="{{ route('admin.permissions.edit', ['permission' => $permission->uuid]) }}"
                                   class="float-right btn btn-primary">Edit</a>
                            @endcan
                        </div>
                    @endcanany
                </div>
