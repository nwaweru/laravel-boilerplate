@extends('ui.layouts.basic', ['title' => 'Edit Role'])

@section('content')
<div class="row">
    <div class="col">
        <form method="POST" action="{{ route('admin.roles.update', ['role' => $role->uuid]) }}" novalidate>
            @method('PUT')
            @csrf
            <div class="row justify-content-center">
                <div class="col-md-4">
                    <div class="card">
                        <div class="card-body">
                            <h3 class="pb-3">Edit Role</h3>
                            <div class="form-group">
                                <label for="name" class="sr-only">Name</label>
                                <input type="text" id="name" name="name"
                                        class="form-control @error('name') is-invalid @enderror"
                                        value="{{ old('name') ?? $role->name }}" required placeholder="Name" autofocus>
                                @error('name')
